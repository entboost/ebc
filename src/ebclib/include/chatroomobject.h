...
public:
	...
	void SetSize(unsigned long size)	// 先生成内存
	{
		if (m_size < size)
		{
			clear();
	...
	void SetData(const char * data)		// 再填充数据
	{
		if (m_size > 0 && m_data != NULL)
		{
...
public:
	...
	void SetMsgId(eb::bigint nMsgId) {m_nMsgId=nMsgId;}
	...
	EB_ChatRoomRichMsg(eb::bigint nMsgId)
		: m_nMsgId(nMsgId)
		, m_nSubType(0)
	{
	}
