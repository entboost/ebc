...
HBRUSH CDialogSK::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	//HBRUSH hBrush = CDialog::OnCtlColor(pDC, pWnd, nCtlColor);
	//if (nCtlColor == CTLCOLOR_STATIC)
	//{
	//	TCHAR cClassName[MAX_PATH] = {0};
	//	if ( 0 != GetClassName(pWnd->GetSafeHwnd(), cClassName, MAX_PATH)
	//		&& 0 != _tcscmp(_T("Edit"), cClassName)
	//		&& 0 != _tcscmp(_T("ComboBox"), cClassName)
	//		&& 0 != _tcscmp(_T("Static"), cClassName))
	//	{
	...
	{  
		...
		return HBRUSH(GetStockObject(/*HOLLOW_BRUSH*/WHITE_BRUSH));
	}
	return hbr;
...
void CDialogSK::Init()
{
	...
	m_nTransType = TT_STATIC | TT_EDIT;
...
void CDialogSK::SetCircle(bool bCircle)
{
	const bool m_bCircle = bCircle;
	if (m_bCircle)
	{
		CRect rectClient;
