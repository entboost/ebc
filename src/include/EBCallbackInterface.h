...
public:
	...
	virtual void onGroupDelete(const EB_GroupInfo* pGroupInfo, bool bIsMyGroup) {}			//
        virtual void onGroupEditResponse(const EB_GroupInfo* pGroupInfo, EB_STATE_CODE nState) {}			//
	...
	virtual void onMemberDelete(const EB_MemberInfo* pMemberInfo, bool bIsMyDefaultMember) {}						// 
        virtual void onMemberEditResponse(const EB_MemberInfo* pMemberInfo, EB_STATE_CODE nState) {}						//
...
typedef enum EB_WINDOW_MESSAGE_TYPE
{
	EB_WM_UNKNOWN				= 0
#ifdef _QT_MAKE_
	, EB_WM_APPID_SUCCESS		= QEvent::User+0x101
#else
...
#ifdef _QT_MAKE_
	, EB_WM_CALL_INCOMING	= QEvent::User+0x201
#else
	...
	// EB_STATE_CODE nState = (EB_STATE_CODE)lp;
#ifdef _QT_MAKE_
	, EB_WM_V_REQUEST_RESPONSE	= QEvent::User+0x211
#else
	...
	// EB_STATE_CODE nState = (EB_STATE_CODE)lp;
#ifdef _QT_MAKE_
	, EB_WM_RD_REQUEST_RESPONSE	= QEvent::User+0x221
#else
...
#ifdef _QT_MAKE_
	, EB_WM_ENTERPRISE_INFO	= QEvent::User+0x301
#else
	...
	// const EB_ContactInfo* pContactInfo = (const EB_ContactInfo*)wp;
#ifdef _QT_MAKE_
	, EB_WM_CONTACT_INFO	= QEvent::User+0x311
#else
	...
	// const EB_APMsgInfo* pApMsgInfo = (const EB_APMsgInfo*)lp;
#ifdef _QT_MAKE_
	, EB_WM_BROADCAST_MSG		= QEvent::User+0x321
#else
...
#ifdef _QT_MAKE_
	, EB_WM_NEW_VERSION			= QEvent::User+0x401
#else
...
#ifdef _QT_MAKE_
	, EB_WM_SERVER_CHENGE		= QEvent::User+0x411
#else
	...
	, EB_WM_STATE_CODE
#ifdef _QT_MAKE_
	, EB_WM_SUBSCRIBE_FUNC_INFO	= QEvent::User+0x501
#else
	, EB_WM_SUBSCRIBE_FUNC_INFO	= WM_USER+0x501
#endif

#ifdef _QT_MAKE_
    , EB_WM_EVENT_RESULT        	= QEvent::User+0x901
#endif
