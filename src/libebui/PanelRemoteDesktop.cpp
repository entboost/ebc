...
void CPanelRemoteDesktop::SetCtrlColor(void)
{
	m_bEnableControl.SetCheckBoxHoverBorder(TRUE,theApp.GetHotColor2());

	m_btnVideoOn.SetDrawPanel(true,theApp.GetOkColor1(),theApp.GetHotColor(),theApp.GetNormalColor());
	m_btnVideoAccept.SetDrawPanel(true,theApp.GetOkColor1(),theApp.GetHotColor(),theApp.GetNormalColor());
	m_btnVideoEnd.SetDrawPanel(true,theApp.GetOkColor1(),theApp.GetHotColor(),theApp.GetNormalColor());
	m_btnVideoCancel.SetDrawPanel(true,theApp.GetOkColor1(),theApp.GetHotColor(),theApp.GetNormalColor());
}
...
BOOL CPanelRemoteDesktop::OnInitDialog()
{
	...
	//m_btnVideoOn.Load(IDB_PNG_71X32G);
	...
	if (m_pCallInfo.m_sGroupCode==0)
	{
		...
		//m_btnVideoAccept.ShowWindow(SW_SHOW)
...
void CPanelRemoteDesktop::RDAckResponse(const EB_RemoteDesktopInfo* pVideoInfo,int nStateValue)
{
	...
	if (m_pCallInfo.m_sGroupCode==0)
	{
		//m_bEnableControl.ShowWindow(pVideoInfo->m_nRDType==EB_RD_DESKTOP_DEST?SW_SHOW:SW_HIDE);
		if (pVideoInfo->m_nRDType==EB_RD_DESKTOP_DEST)
			m_bEnableControl.SetWindowText(_T("允许对方控制本机"));
		else
			m_bEnableControl.SetWindowText(_T("允许控制对方电脑"));
