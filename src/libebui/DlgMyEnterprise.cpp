...
void CDlgMyEnterprise::OnImportEntData()
{
	...
	{
		const std::string sUrl = theApp.GetEntManagerurl();
		char * lpszBuffer = new char[sUrl.size()+1];
		strcpy(lpszBuffer,sUrl.c_str());
		this->GetParent()->PostMessage(EB_COMMAND_OPEN_APP_URL,(WPARAM)lpszBuffer,(LPARAM)0);
		
		//ShellExecuteA(NULL, "open", theApp.GetEntManagerurl().c_str(), NULL, NULL,SW_SHOW);

		//CWnd * pParent = CWnd::FromHandle(::GetDesktopWindow());
		//CDlgFuncWindow * pFuncWindow = new CDlgFuncWindow(pParent,true);
		//pFuncWindow->m_sTitle = _T("管理中心");
		//pFuncWindow->m_sFuncUrl = m_sEntManagerUrl;
		//pFuncWindow->m_nWidth = 0;
		//pFuncWindow->m_nHeight = 0;
		//pFuncWindow->m_bDisableContextMenu = true;
		//pFuncWindow->m_bDisableScrollBar = true;
		//pFuncWindow->m_bClosePrompt = true;
...
void CDlgMyEnterprise::OnDepartmentDelete()
{
	...
	{
		CDlgMessageBox::EbMessageBox(this,"存在下级子部门：",_T("不能删除部门资料！"),CDlgMessageBox::IMAGE_WARNING,3);
		return;
	}
	...
	//	return;
	//}
	const CString sTitle = bHasMemberInfo?_T("你确定删除部门和部门所有成员资料："):_T("你确定删除部门资料：");
	CString sText;
	sText.Format(_T("[%s]吗？"), pDepItemInfo->m_sName.c_str());
...
void CDlgMyEnterprise::OnEmployeeDelete()
{
	...
	if (CDlgMessageBox::EbDoModal(this,"你确定删除员工资料：",sText,CDlgMessageBox::IMAGE_QUESTION)==IDOK)
...
void CDlgMyEnterprise::DepartmentInfo(IEB_GroupInfo* pGroupInfo)
{
	...
	pTreeItemInfo->m_nSubType = pGroupInfo->GroupType;
	m_treeEnterprise.Sort(hParentItem,CEBUIApp::TreeCmpFunc);
...
void CDlgMyEnterprise::EmployeeInfo(IEB_MemberInfo* pEBEmployeeInfo, bool bChangeLineState)
{
	...
	if (!m_pEmpItemInfo.find(sMemberCode,pEmpItemInfo))
	{
		...
		m_treeEnterprise.Sort(pDepItemInfo->m_hItem,CEBUIApp::TreeCmpFunc);
	}else
	{
		pEmpItemInfo->m_sName = sUserName;
		pEmpItemInfo->m_dwItemData = pEBEmployeeInfo->LineState;
...
//			}
...
#endif
	{
		if (pTreeItemInfo->m_nItemType==CTreeItemInfo::ITEM_TYPE_ENTERPRISE || pTreeItemInfo->m_nItemType==CTreeItemInfo::ITEM_TYPE_GROUP)
		{
			if (bNeedSeparator)
				m_menu2.AppendMenu(MF_SEPARATOR);
			bNeedSeparator = true;
			m_menu2.AppendMenu(MF_BYCOMMAND,EB_COMMAND_NEW_DEPARTMENT1,_T("新建部门"));
			m_menu2.AppendMenu(MF_BYCOMMAND,EB_COMMAND_NEW_DEPARTMENT2,_T("新建项目组"));
		}
		if (pTreeItemInfo->m_nItemType==CTreeItemInfo::ITEM_TYPE_GROUP)
		{
			if (bNeedSeparator)
...
#endif
	{
		...
		{
			CString sText;
			sText.Format(_T("删除%s"),GetGroupTypeText((EB_GROUP_TYPE)pTreeItemInfo->m_nSubType));
