...
void CDlgResourceMgr::SetCtrlColor(void)
{
	m_treeDir.SetBkGradients(theApp.GetBgColor2(),theApp.GetBgColor2());
...
BOOL CDlgResourceMgr::OnInitDialog()
{
	...
	this->SetToolTipText(IDC_TREE_DIR,_T("点击编辑，回车保存！"));
	//this->SetToolTipText(IDC_TREE_ITEM,_T("双击下载文件"));
	...
	m_btnNewDir.SetAutoSize(false);
	m_btnNewDir.SetNorTextColor(RGB(0,128,255));
	m_btnNewDir.SetHotTextColor(RGB(0,128,255));
	m_btnRefresh.SetAutoSize(false);
	m_btnRefresh.SetNorTextColor(RGB(0,128,255));
	m_btnRefresh.SetHotTextColor(RGB(0,128,255));
	m_btnNewFile.SetAutoSize(false);
	...
	m_treeDir.SetCallback((CTreeCallback*)this);
	m_treeDir.ModifyStyle(0, TVS_SINGLEEXPAND);
	m_treeDir.SetTreeOpenClosedBmp(IDB_TREE_OPENED, IDB_TREE_CLOSED);
	m_treeDir.SetItemHeight(40);
	m_treeDir.SetIconSize(32,32);
	...
	m_btnHotNewSubDir.Load(IDB_PNG_HOT_NEW);
	...
	m_treeItem.SetCallback((CTreeCallback*)this);
	m_treeItem.ModifyStyle(0, TVS_SINGLEEXPAND);
	//m_treeItem.SetTreeOpenClosedBmp(IDB_TREE_OPENED, IDB_TREE_CLOSED);
...
void CDlgResourceMgr::OnSize(UINT nType, int cx, int cy)
{
	...
	const int const_btn_width = 50;		//m_btnNewDir.GetImgWidth();
	const int const_btn_height = 24;	//m_btnNewDir.GetImgHeight();
	...
	x = cx-m_btnNewFile.GetImgWidth()-8;	// 最右边间隔
	m_btnNewFile.MovePoint(x,y-2);
...
void CDlgResourceMgr::OnPaint()
{
	...
	else
	{
		...
		this->AddBgDrawInfo(CEbBackDrawInfo(39,theDefaultBgColorTitleLight0,false));
...
#else
void CDlgResourceMgr::OnResourceMove(const EB_ResourceInfo& pResourceInfo,const std::string& sOldParentResId)
{
	CTreeItemInfo::pointer pTreeItemInfo;
...
void CDlgResourceMgr::OnResourceInfo(const EB_ResourceInfo& pResourceInfo)
{
	...
	if (pResourceInfo.m_nResType==EB_RESOURCE_DIR)
	{
		...
		CTreeItemInfo::pointer pTreeItemInfo;
		if (!m_pDirItemInfo.find(pResourceInfo.m_sResId,pTreeItemInfo))
		{
	...
	*pResult = 0;
	...
	if (!theApp.IsLogonVisitor())
	{
		...
		if ((m_nManagerType==RES_MANAGER_GROUP && bCanEditGroupRes) || (m_nManagerType==RES_MANAGER_MY))
			m_menu2.AppendMenu(MF_BYCOMMAND,EB_COMMAND_NEW_DIR_RES,_T("添加目录"));
		...
		if (pTreeItemInfo!=NULL && pTreeItemInfo!=m_pAllResParent.get())
		{
			if ((m_nManagerType==RES_MANAGER_GROUP && bCanEditGroupRes) ||
				(m_nManagerType==RES_MANAGER_MY))
			{
				m_menu2.AppendMenu(MF_BYCOMMAND,EB_COMMAND_NEW_SUBDIR_RES,_T("添加子目录"));
...
void CDlgResourceMgr::OnNMRClickTreeItem(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	if (!theApp.IsLogonVisitor())
	{
		...
		if ((m_nManagerType==RES_MANAGER_GROUP && bCanEditGroupRes) || (m_nManagerType==RES_MANAGER_MY))
		{
			m_menu2.AppendMenu(MF_BYCOMMAND,EB_COMMAND_NEW_FILE_RES,_T("添加云文件"));
				...
				(m_nManagerType==RES_MANAGER_MY))
			{
				if (!m_pDirItemInfo.empty())
				{
					...
					for (;pIter!=m_pDirItemInfo.end();pIter++)
					{
						...
						if (pDirTreeItemInfo->m_sId != pTreeItemInfo->m_sParentId)
						{
...
void CDlgResourceMgr::OnNewDirRes(void)
{
	...
	if ((m_nManagerType==RES_MANAGER_GROUP && bCanEditGroupRes) || (m_nManagerType==RES_MANAGER_MY))
	{
		m_hNewDirTempItem = m_treeDir.InsertItem(_T("新建目录"));
		m_treeDir.ModifyStyle(0, TVS_EDITLABELS);
		m_treeDir.EditLabel(m_hNewDirTempItem);
	}else
	{
		CDlgMessageBox::EbMessageBox(this,"对不起，你没有操作权限！",_T(""),CDlgMessageBox::IMAGE_ERROR);
	}
...
#endif
	{
		CDlgMessageBox::EbDoModal(this,_T("不能删除非空目录！"),_T(""),CDlgMessageBox::IMAGE_ERROR);
		return;
	}

	CString sText;
	sText.Format(_T("[%s]吗？"), pTreeItemInfo->m_sName.c_str());
	if (CDlgMessageBox::EbDoModal(this,"你确定删除：",sText,CDlgMessageBox::IMAGE_QUESTION)==IDOK)
...
void CDlgResourceMgr::NewItem(IEB_ResourceInfo* pResourceInfo)
{
	...
	if (nResType==EB_RESOURCE_FILE)
	{
		...
		if (m_sGroupCode==0)
		{
			sItemName.Format(_T("%s\n%s  %s  %d次下载"),sName.c_str(),sTime.c_str(),sSize,nDownloads);
		}else
		{
			...
			if (pResourceInfo->Share==EB_RESOURCE_SHARE_TEMP)
				sItemName.Format(_T("%s  （群组临时文件，上传者：%s）\n%s  %s  %d次下载"),sName.c_str(),sMemberName.c_str(),sTime.c_str(),sSize,nDownloads);
			else
...
void CDlgResourceMgr::NewItem(const EB_ResourceInfo& pResourceInfo)
{
	...
	//{
	//}
	std::string sTime(pResourceInfo.m_sTime);
	...
	if (pResourceInfo.m_nResType==EB_RESOURCE_FILE)
	{
		...
		if (m_sGroupCode==0)
		{
			sItemName.Format(_T("%s\n%s  %s  %d次下载"),pResourceInfo.m_sName.c_str(),sTime.c_str(),sSize,pResourceInfo.m_nDownloads);
		}else
		{
			...
			if (pResourceInfo.m_nShare==EB_RESOURCE_SHARE_TEMP)
				sItemName.Format(_T("%s  （群组临时文件，上传者：%s）\n%s  %s  %d次下载"),pResourceInfo.m_sName.c_str(),sMemberName.c_str(),sTime.c_str(),sSize,pResourceInfo.m_nDownloads);
			else
				sItemName.Format(_T("%s  （上传者：%s）\n%s  %s  %d次下载"),pResourceInfo.m_sName.c_str(),sMemberName.c_str(),sTime.c_str(),sSize,pResourceInfo.m_nDownloads);
		}
	...
	CTreeItemInfo::pointer pTreeItemInfo;
	if (!m_pItemItemInfo.find(pResourceInfo.m_sResId,pTreeItemInfo))
	{
		HTREEITEM hItem = m_treeItem.InsertItem(sItemName);
...
void CDlgResourceMgr::DownloadFile(HTREEITEM hItem)
{
	...
	AFX_SWITCH_INSTANCED();
	CFileDialog dlg(FALSE, 0, pTreeItemInfo->m_sName.c_str(), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, theAllFilesFilter, this);
	if (dlg.DoModal() == IDOK)
	{
...
void CDlgResourceMgr::OnTvnEndlabeleditTreeDir(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	{
		CDlgMessageBox::EbDoModal(this,_T("目录名称不能为空："),_T("请重新输入！"),CDlgMessageBox::IMAGE_ERROR);
