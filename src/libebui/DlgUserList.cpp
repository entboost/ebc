...
void CDlgUserList::LineStateChange(eb::bigint nUserId, EB_USER_LINE_STATE bLineState)
{
	...
	if (m_pUserItem.find(nUserId,pTreeItemInfo))
	{
		if (pTreeItemInfo->m_dwItemData != bLineState)
		{
			...
			m_treeUsers.Sort(TVI_ROOT, CEBUIApp::TreeCmpFunc);
		}
	}
}
...
void CDlgUserList::OnEnterUser(eb::bigint nUserId)
{
...
#ifdef USES_EBCOM_TEST
void CDlgUserList::OnUserEmpInfo(IEB_MemberInfo* pMemberInfo)
{
...
#else
void CDlgUserList::OnUserEmpInfo(const EB_MemberInfo* pMemberInfo)
{
	if (pMemberInfo==NULL) return;
	CTreeItemInfo::pointer pTreeItemInfo;
	CString sText;
...
void CDlgUserList::SetCtrlColor(void)
{
	m_treeUsers.SetBkGradients(theApp.GetBgColor2(),theApp.GetBgColor2());
}
...
BOOL CDlgUserList::OnInitDialog()
{
	...
	m_btnEditEmpTrack.SetToolTipText(_T("修改我的群名片"));
	...
	//m_treeUsers.SetRootIcon( m_icon );
	//m_treeUsers.SetItemIcon( m_icon );
...
{
	const CTreeItemInfo * pTreeItemInfo = hItem==NULL?NULL:(const CTreeItemInfo*)m_treeUsers.GetItemData(hItem);
...
void CDlgUserList::OnNMRClickTreeUsers(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	if (nGroupCreateUserId==theApp.GetLogonUserId())
	{
		...
		case EB_GROUP_TYPE_DEPARTMENT:
		case EB_GROUP_TYPE_PROJECT:
			m_menu2.AppendMenu(MF_BYCOMMAND,EB_COMMAND_EDIT_DEPARTMENT,_T("修改员工资料"));
			break;
		case EB_GROUP_TYPE_GROUP:
