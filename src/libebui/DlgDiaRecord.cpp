...
/*
	...
	if (pResltSet != NULL)
	{
		for (int i=0; i<pResltSet->rscount; i++)
		{
			...
			std::string sAccount = pResltSet->rsvalues[i]->fieldvalues[5]->v.varcharVal.buffer;
			std::string sDepCode = pResltSet->rsvalues[i]->fieldvalues[6]->v.varcharVal.buffer;
			std::string sText = pResltSet->rsvalues[i]->fieldvalues[7]->v.varcharVal.size>0?pResltSet->rsvalues[i]->fieldvalues[7]->v.varcharVal.buffer:"";
