	...
	case RECO_PASTE:  
		{
			...
			if (bFileAvailable)
			{
				...
				int nFileSum = DragQueryFile(hDrop,0xFFFFFFFF,NULL,0);   //得到文件个数
...
COleRichEditCtrl::IExRichEditOleCallback::GetClipboardData(CHARRANGE FAR *lpchrg, DWORD reco, LPDATAOBJECT FAR *lplpdataobj)
{
	...
	//	string   strCodedText=ToCodedString(* lpchrg,strText);  
		const std::string strCodedText(strText);
														 ...
														 HMENU FAR *lphmenu)
{
	...
	if (m_pMenu.GetSafeHmenu()==NULL)
	{
		...
		m_pMenu.AppendMenu(0, ID_RICH_UNDO, TEXT("撒销(&U)\tCtrl+Z"));
		//m_pMenu.AppendMenu(0, ID_RICH_REDO, TEXT("重做(&O)\tCtrl+Y"));
		m_pMenu.AppendMenu(MF_SEPARATOR);
		m_pMenu.AppendMenu(0, ID_RICH_CUT, TEXT("剪切(&X)\tCtrl+X"));
		m_pMenu.AppendMenu(0, ID_RICH_COPY, TEXT("复制(&C)\tCtrl+C"));
		m_pMenu.AppendMenu(0, ID_RICH_PASTE, TEXT("粘贴(&V)\tCtrl+V"));
		m_pMenu.AppendMenu(MF_SEPARATOR);
		m_pMenu.AppendMenu(0, ID_RICH_SELALL, TEXT("全选(&A)\tCtrl+A"));
		//m_pMenu.AppendMenu(MF_SEPARATOR);
