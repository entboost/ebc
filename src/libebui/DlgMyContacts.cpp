...
void CDlgMyContacts::ContactInfo(IEB_ContactInfo* pPopContactInfo)
{
	...
	m_treeContacts.Sort(pGroupItemInfo->m_hItem, CEBUIApp::TreeCmpFunc);
}
#else
...
void CDlgMyContacts::DeleteItem(HTREEITEM hItem)
{
	...
	CString sText;
	if (theApp.GetAuthContact())
		sText.Format(_T("[%s(%s)]，同时从对方联系人列表中消失吗？"), pContactItemInfo->m_sName.c_str(), pContactItemInfo->m_sAccount.c_str());
	else
...
{
	m_hEditGroupTempItem = m_treeContacts.InsertItem(_T("未命名"));
...
void CDlgMyContacts::OnGroupDelete()
{
	...
	CString sText;
	sText.Format(_T("[%s] 并将该分组所有联系人移到默认分组吗？"), pGroupItemInfo->m_sName.c_str());
	if (CDlgMessageBox::EbDoModal(this,"你确定删除分组：",sText,CDlgMessageBox::IMAGE_QUESTION)==IDOK)
	{
#ifdef USES_EBCOM_TEST
...
void CDlgMyContacts::OnContactNew()
{
	if (theApp.GetAuthContact())
	{
		...
		if (theApp.GetFindAppSugId()==0)
			CDlgMessageBox::EbDoModal(this,_T("暂时未开放添加好友验证功能："),_T("请联系客服！"),CDlgMessageBox::IMAGE_ERROR);
		else
		{
			// 找群找人
#ifdef USES_EBCOM_TEST
	...
	*pResult = 0;
	...
	if (!theApp.IsLogonVisitor())
	{
		...
		}else if (pTreeItemInfo->m_nItemType==CTreeItemInfo::ITEM_TYPE_CONTACT)
		{
			...
			m_menu2.AppendMenu(MF_SEPARATOR);
			m_menu2.AppendMenu(MF_BYCOMMAND,EB_COMMAND_NEW_CONTACT,_T("添加联系人"));
			m_menu2.AppendMenu(MF_BYCOMMAND,EB_COMMAND_EDIT_CONTACT,_T("修改联系人"));
			if (m_pGroupItemInfo.size()>1)
			{
				...
				for (; pIter!=m_pGroupItemInfo.end();  pIter++)
				{
					...
					if (pGroupItemInfo->m_sId==const_default_group_ugid || pGroupItemInfo->m_sId==pTreeItemInfo->m_sParentId)
						continue;
					pPopupMenu.AppendMenu(MF_BYCOMMAND,EB_COMMAND_MY_UGINFO+nIndex,pGroupItemInfo->m_sName.c_str());
					nIndex++;
				}
				m_menu2.AppendMenu(MF_POPUP|MF_BYPOSITION,(UINT)pPopupMenu.m_hMenu,_T("移动联系人至"));
			}
		...
		}else if (pTreeItemInfo->m_nItemType==CTreeItemInfo::ITEM_TYPE_GROUP)
		{
			...
			if (const_default_group_ugid!=pTreeItemInfo->m_sId)
			{
				...
				m_menu2.AppendMenu(MF_BYCOMMAND,EB_COMMAND_DELETE_GROUP,_T("删除该组"));
			}
		}
...
void CDlgMyContacts::OnTvnEndlabeleditTreeContacts(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	{
		CDlgMessageBox::EbDoModal(this,_T("分组不能为空："),_T("请重新输入！"),CDlgMessageBox::IMAGE_ERROR);
