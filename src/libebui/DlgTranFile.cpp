...
BOOL CDlgTranFile::OnInitDialog()
{
	...
	const std::string sFileName = libEbc::GetFileName(m_pCrFileInfo.m_sFileName);
	CString sFileText;
	...
	//}else
	{
		if (m_bIsSendingFile)
		{
			...
			this->GetDlgItem(IDC_BUTTON_SAVE)->ShowWindow(SW_HIDE);
			this->GetDlgItem(IDC_BUTTON_REJECT)->ShowWindow(SW_HIDE);
			if (m_sResourceId>0 || m_pCrFileInfo.m_bOffFile)
				this->GetDlgItem(IDC_BUTTON_OFFFILE)->ShowWindow(SW_HIDE);
			else
...
void CDlgTranFile::OnSize(UINT nType, int cx, int cy)
{
	...
	if (m_sResourceId>0 || m_bIsSendingFile)
	{
...
void CDlgTranFile::OnBnClickedButtonSaveas()
{
	...
	CFileDialog dlg(FALSE, 0, m_pCrFileInfo.m_sFileName.c_str(), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, 0, this);
	if (dlg.DoModal() == IDOK)
	{
		m_btnSave2CloutDrive.ShowWindow(SW_HIDE);
...
#ifdef USES_EBCOM_TEST
	if (m_sResourceId>0)
		theEBClientCore->EB_CancelFileRes(m_sResourceId,m_pCrFileInfo.m_nMsgId);
	else
		theEBClientCore->EB_CancelFile(m_pCrFileInfo.GetCallId(),m_pCrFileInfo.m_nMsgId);
#else
	if (m_sResourceId>0)
		theEBAppClient.EB_CancelFileRes(m_sResourceId,m_pCrFileInfo.m_nMsgId);
	else
		theEBAppClient.EB_CancelFile(m_pCrFileInfo.GetCallId(),m_pCrFileInfo.m_nMsgId);
#endif
}
...
void CDlgTranFile::SetFilePercent(const CChatRoomFilePercent * pChatRoomFilePercent)
{
	...
	CString sSheechStatus;
	if (pChatRoomFilePercent->m_nCurSpeed>1024000.0)
